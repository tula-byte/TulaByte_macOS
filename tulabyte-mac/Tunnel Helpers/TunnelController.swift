...
let tunnelSafetyOverrideKey = "tunnelSafetyOverride"
let tunnelLocalizedDescription = "TulaByte"

class TunnelController: NSObject {
    
    static let shared = TunnelController()
    let tunnelEnabledNotification: NSNotification = NSNotification(name: NSNotification.Name("TulaByteTunnelProviderEnabled"), object: nil)
    let tunnelDisabledNotification: NSNotification = NSNotification(name: NSNotification.Name("TulaByteTunnelProviderDisabled"), object: nil)
        ...
        NETunnelProviderManager.loadAllFromPreferences { (managers, error) in
            ...
            if let managers = managers, managers.count > 0 {
                ...
                if self.manager == managers[0] {
                    ...
                    NSLog("TB MAC:Existing manager is the same as the one being created. Not Replacing.")
                    completion(nil)
                }
    ...
    func tunnelStatus() -> NEVPNStatus {
        ...
        if manager != nil {
            ...
            NSLog("TB MAC: No Tunnel Manager Found")
            // return invalid
            return .invalid
        }
    }
    ...
    func deleteManager() {
        ...
        refreshManager { (error) in
            ...
            self.manager?.removeFromPreferences(completionHandler: { (removalError) in
                if removalError != nil {
                    NSLog("TB MAC: Error while deleting manager. \(String(describing: removalError))")
                } else {
                    NSLog("TB MAC: Manager Deleted.")
                }
            })
            }
        }
    ...
    func restartTunnel(completion: @escaping (_ error: Error?) -> Void = {_ in }) {
        ...
        TunnelController.shared.enableTunnel(false) { (disableError) in
            ...
            if disableError != nil {
                // ...log it
                NSLog("TB MAC:There was error disabling the tunnel and IDK what to do about it: \(disableError!)")
            }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                ...
                TunnelController.shared.enableTunnel(true) { (enableError) in
                    ...
                    if enableError != nil {
                        // ...log it
                        NSLog("TB MAC:There was error enabling the tunnel and IDK what to do about it: \(enableError!)")
                    }
    ...
    func setupTunnel(completion: @escaping (_ error: Error?) -> Void = {_ in }) {
        ...
        NETunnelProviderManager.loadAllFromPreferences { (managers, error) in
            ...
            if let managers = managers, managers.count > 0 {
                ...
                providerProtocol.providerBundleIdentifier = "com.tulabyte.tulabyte.tunnelProviderAppex"
            ...
            self.manager!.isEnabled = true // set status of tunnel based on the function variable
            self.manager!.isOnDemandEnabled = true // set background enable based on the function variable
            ...
            self.manager!.saveToPreferences { (error) in
                ...
                if let e = error as? NEVPNError {
                    NSLog("TB MAC: Error in the VPN manager while enabling it for the first time")
    ...
    func enableTunnel(_ enabled: Bool, isUserExplicitToggle: Bool = false, completion: @escaping (_ error: Error?) -> Void = {_ in }) {
        if (defaults.value(forKey: tunnelSafetyOverrideKey) as! Bool) == true {
            NSLog("TB MAC: Tunnel setup safety overridden")
            defaults.setValue(false, forKey: tunnelSafetyOverrideKey)
        } else {
        NSLog("TB MAC: Firewall tunnel status: \(enabled)")
        ...
        if isUserExplicitToggle {
            NSLog("TB MAC: User requested tunnel state change to \(enabled)")
        ...
        NETunnelProviderManager.loadAllFromPreferences { (managers, error) in
            ...
            self.manager!.saveToPreferences { (error) in
                // check if there is an error specifically of the NEVPNError class
                if let e = error as? NEVPNError {
                    NSLog("TB MAC: Error in the VPN manager while changing state to \(enabled): \(e)")
                    // if any of the following are found, break the switch
                    NSLog("TB MAC: Exited early since the user denied permission")
                    completion(e)
                    switch e.code {
                    case .configurationDisabled:
                        break;
                ...
                // if its a regular error, then just log it
                    ...
                    NSLog("TB MAC: There was error saving the VPN config when changing state to \(enabled): \(e)")
                }
                // if nothing goes wrong
                else {
                    NSLog("TB MAC: Succesfully saved the VPN state change to \(enabled)")
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                        do {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                                ...
                                if TunnelController.shared.tunnelStatus() == .connected {
                                    NotificationCenter.default.post(self.tunnelEnabledNotification as Notification)
                                } else if TunnelController.shared.tunnelStatus() == .disconnected {
                                    NotificationCenter.default.post(self.tunnelDisabledNotification as Notification)
                                }
                            }
                        }
                        // if there is an error then log it and go on your merry way
                        catch {
                            NSLog("TB MAC: Unable to start the tunnel after saving: " + error.localizedDescription)
                        }
                    }
