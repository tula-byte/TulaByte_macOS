...
let purchasedNotification = NSNotification(name: NSNotification.Name(hasUserPurchasedKey), object: nil)
...
func isSubscriptionValid() -> Bool {
    ...
    do {
        ...
        os_log("TB SK: Receipt couldn't be found.")
        return false
    }
...
func isSubscriptionValidRefreshed() -> Bool {
    ...
    InAppReceipt.refresh { (error) in
        if error != nil {
            os_log("TB SK: Receipt could not be refreshed. Error: \(String(describing: error))")
        } else {
            do {
                try receipt = InAppReceipt.localReceipt()
            } catch {
    ...
    var activeSub: Bool {
        if ((receipt.activeAutoRenewableSubscriptionPurchases(ofProductIdentifier: IndividualSubscription, forDate: Date()) != nil) || (receipt.activeAutoRenewableSubscriptionPurchases(ofProductIdentifier: FamilySubscription, forDate: Date()) != nil)) {
            os_log("TB SK: Found valid subscription")
            return true
        } else {
            os_log("TB SK: No valid subscription")
            return false
        }
    }
    ...
    if receipt != nil && activeSub == true {
        do {
            //verify the receipt
            try receipt.verify()
            os_log("TB SK: Receipt Validated Good")
            return true
        } catch {
            os_log("TB SK: Invalid Receipt")
            return false
